...
export class MovieService {
    ...
    */
    ...
    movieUpdate(id, updatedMovie){
        let params = JSON.stringify(updatedMovie);
        let options = {
            headers: new HttpHeaders({'Content-Type': 'application/json'})
        };
        return this._http.put(
            this.url + 'pelicula/' + id,//ESTA ES LA RUTA EN EL BACK
            params,
            options
        ).pipe(map(res => res));
    }
    ...
    searchMovies(key, value){
        let options = {
            headers: new HttpHeaders( {'Content-Type': 'application/json'} )
        };
        return this._http.get(
            this.url + "buscarPelicula/"+key+"="+value,
            options
        ).pipe(map(res => res));
    }

    uploadMovieImage(file: File, id){
        var formData = new FormData();
        formData.append('imageFile', file);
        console.log(formData)
        return this._http.put(
            this.url + 'imagenPelicula/' + id,
            formData
        ).pipe(map(res => res))
    }
    uploadMovieVideo(file: File, id){
        var formData = new FormData();
        formData.append('videoFile', file);
        console.log(formData)
        return this._http.put(
            this.url + 'videoPelicula/' + id,
            formData
        ).pipe(map(res => res))
    }
}